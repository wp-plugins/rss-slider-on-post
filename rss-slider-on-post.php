...
Description: Use this plugin to scroll the content vertically in the posts and pages.
Author: Gopi.R
Version: 1.0
...
Tags: Wordpress, plugin, Rss, Slider
*/
...
function rssslider_install() 
{
	...
	add_option('rss_s1', "http://www.gopiplus.com/work/feed");
...
<li>Check official website for more info and live demo <a href="http://www.gopiplus.com/work/2012/04/01/rss-slider-on-post-wordpress-plugin/" target="_blank">click here</a></li>
</ul>
<?php
}

add_filter('the_content','rssslider_Show_Filter');

function rssslider_Show_Filter($content)
{
	return 	preg_replace_callback('/\[RSS-SLIDER=(.*?)\]/sim','rssslider_Show_Filter_Callback',$content);
}

function rssslider_Show_Filter_Callback($matches) 
{
	global $wpdb;
	//[RSS-SLIDER=SETTING1]
	$scode = $matches[1];
...
add_action('admin_menu', 'rssslider_add_to_menu');
add_action('init', 'rssslider_add_javascript_files');
?>